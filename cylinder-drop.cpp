...
void post_step_callback(Simulator* sim)
{
  ...
  std::ofstream out("CylinderData01.txt", std::ostream::app);
  out << coords[0] << " " <<coords[1] << " " <<coords[2] <<" " <<coords[3]<<" " <<coords[4]<<" " <<coords[5] << " " <<sim->current_time<<std::endl;
  out.close();
}
...
void init(void* separator, const std::map<std::string, Moby::BasePtr>& read_map, double time)
{
  ...
  std::ofstream out("CylinderData01.txt");
  out.close();
