...
if len(sys.argv) < 2:
  print 'syntax: plotCylinderData.py <joint name> <direction>'
  sys.exit(0)  
...
DT001 = 0.001
# load data
y2 = pylab.loadtxt(sys.argv[1] + "Control.txt")
y3 = pylab.loadtxt(sys.argv[1] + "01.txt")
y4 = pylab.loadtxt(sys.argv[1] + "05.txt")
y5 = pylab.loadtxt(sys.argv[1] + "10.txt")

t10  = np.linspace(0,  y5.size*DT10,y5.size)
...
# plot data
plt.plot(t001,y2,'k' ,label='Control-0.001')
plt.plot(t01, y3,'g' ,label='0.01')
plt.plot(t05, y4,'b' ,label='0.05')
plt.plot(t10, y5,'b' ,label='0.10')

# set limits
axPlot = plt.subplot(111)
axPlot.set_xlim(-.001, 3)
s = 'Kinetic Energy of Rod in Simulation' + ;
